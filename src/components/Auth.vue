<template>
  <div>
    ...
    <!-- <div v-if="backend_type=='worktile'">
...
async function authorize() {
  if (backend_type.value == "wekan"){
    try{
      ...
      token.value =  await loginUser(baseURL.value,{email:username.value,password:password.value},api_key.value)
      else token.value = await  loginUser(baseURL.value,{username:username.value,password:password.value},api_key.value)
